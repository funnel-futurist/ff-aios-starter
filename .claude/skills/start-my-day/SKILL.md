...
- If `OPERATOR_FILE=present`: read `.aios/local-operator.json`, greet them by name, and sanity-check that `GH_USER` / `GIT_EMAIL` line up with it. If they clearly **don't** match, say so plainly: *"this machine is signed in as X but the operator file says Y — is that right?"* (Don't pretend this is verified identity — it's a label.)
...
- If on a task branch already: tell them which one and ask *"continue this, or start something new?"* New → Step 3 then 4. Continue → Step 3 (offer to pull main's latest into it via a safe `git merge origin/main`), then Step 5.
...
## Step 5 — Pin down the work (30 seconds — prevents building the wrong thing)
Get short answers, reflect them back:
