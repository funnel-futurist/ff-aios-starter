...
# Deterministically plant python bytecode and force it past .gitignore, so what is under
# test is the package spec's exclusion rather than the platform's bytecode behaviour.
mkdir -p "$SRC/scripts/aios/aioslib/__pycache__"
printf '\x00fake bytecode' > "$SRC/scripts/aios/aioslib/__pycache__/util.cpython-312.pyc"
git -C "$SRC" add -Af "$SRC/scripts/aios/aioslib/__pycache__" >/dev/null 2>&1
...
step "generated bytecode never enters a package"
# Running the tool writes .pyc files next to the source on most platforms. If those ever ship,
# they collide with the bytecode the workspace generates itself and the client can never
# upgrade. This passed on macOS for the wrong reason (its system Python redirects bytecode to
# a central cache) and failed on Linux CI, which is the whole argument for a clean runner.
git -C "$SRC" ls-files | grep -q "__pycache__" \
  && ok "the source really does track bytecode (so this assertion can fail)" \
  || bad "no tracked bytecode - this assertion would pass vacuously"
if python3 -c "
import json,sys
m=json.load(open('$WORK/v9.1.0.json'))
bad=[f['path'] for f in m['files'] if '__pycache__' in f['path'] or f['path'].endswith(('.pyc','.pyo'))]
sys.exit(1 if bad else 0)"; then
  ok "no bytecode in the 9.1.0 package"
else
  bad "the package would ship python bytecode"
fi
